...
describe('Core Service E2E', () => {
  ...
  it('should handle the full lifecycle: start, stream, stop, fetch history, and clear', async () => {
    ...
    const startResponse = logger.messages.find(m => m.action === 'server.request_response');
    expect(startResponse?.success).toBe(true);
    const processId = startResponse?.data.id;
    expect(processId).toBeDefined();

    const outputMessages = logger.messages.filter(m => m.action === 'server.process_output');
    ...
    const stoppedMessage = logger.messages.find(m => m.action === 'server.process_stopped');
    if (stoppedMessage?.action !== 'server.process_stopped') throw new Error('Type guard failed');
    ...
    const outputResponse = historyChecker.messages.find(m => m.action === 'server.output_response');
    ...
    await new Promise(resolve => setTimeout(resolve, 100));
    const clearResponse = clearClient.messages.find(m => m.action === 'server.request_response');
    ...
    const finalState = await new Promise<ServerMessage>(resolve => {
        verifier.on('message', data => resolve(JSON.parse(data.toString())));
    });
