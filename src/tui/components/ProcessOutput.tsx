...
import {
    Box, measureElement, Text, useStdout,
} from 'ink';
...
const ProcessOutput: FC<ProcessOutputProps> = ({ process }) => {
    ...
    const renderContent = (): React.JSX.Element => {
        ...
        if (process.output) {
            ...
            if (truncatedCount > 0) {
                ...
                const messageHeight = Math.ceil(
                    stringWidth(truncationMessage) / width,
                );

                // Remove lines from the top until the message fits.
                while (
                    currentHeight + messageHeight > height
